const int maxN = 1e6 + 10; // the maximum number of elements in the array on which segment tree is to be build
...
void build( int si, int ss, int se ) {
	...
	st[si] = min(st[si*2], st[si*2+1]); // this is the logic of the segment tree, here it is used to build a min segment tree, change this according to the needs of the question
}
...
int query( int si, int ss, int se, int qs, int qe ) {
	...
	return (min(query( 2 * si, ss, mid, qs, qe ), query( 2 * si + 1, mid + 1, se, qs, qe))); // use the value of the left and right child to find the value of the required range
}
