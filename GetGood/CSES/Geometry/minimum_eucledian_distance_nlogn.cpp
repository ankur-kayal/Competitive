...
int64_t calculate_distance(vector<P>& points, vector<P>& points_by_y) {
    int n = points.size();
    if(n == 1) {
    ...
    for(P p: points_by_y) {
        if(mp(p.x, p.y) <= mp(left.back().x, left.back().y)) {
    ...
    int64_t left_distance = calculate_distance(left, left_by_y);
    int64_t right_distance = calculate_distance(right, right_by_y);
...
void run_cases() {
    ...
    sort(all(points_by_y), [&](const P& a, const P& b) {
        return a.y < b.y;
    });

    cout << calculate_distance(points, points_by_y);
