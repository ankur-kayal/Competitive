#include <bits/stdc++.h>
using namespace std;

//----------------------------------- DEBUG -----------------------------------
#define sim template < class c
#define ris return * this
#define dor > debug & operator <<
#define eni(x) sim > typename \
enable_if<sizeof dud<c>(0) x 1, debug&>::type operator<<(c i) {
sim > struct rge { c b, e; };
sim > rge<c> range(c i, c j) { return rge<c>{i, j}; }
sim > auto dud(c* x) -> decltype(cerr << *x, 0);
sim > char dud(...);
struct debug {
#ifdef LOCAL
~debug() { cerr << endl; }
eni(!=) cerr << boolalpha << i; ris; }
eni(==) ris << range(begin(i), end(i)); }
sim, class b dor(pair < b, c > d) {
  ris << "(" << d.first << ", " << d.second << ")";
}
sim dor(rge<c> d) {
  *this << "[";
  for (auto it = d.b; it != d.e; ++it)
	*this << ", " + 2 * (it == d.b) << *it;
  ris << "]";
}
#else
sim dor(const c&) { ris; }
#endif
};
#define imie(...) " [" << #__VA_ARGS__ ": " << (__VA_ARGS__) << "] "
// debug & operator << (debug & dd, P p) { dd << "(" << p.x << ", " << p.y << ")"; return dd; }

//----------------------------------- END DEBUG --------------------------------

const int maxN = 3e5+10;
const int logN = ceil(log2(maxN));

vector<vector<int>> adj(maxN,vector<int>());
vector<vector<int>> lca(maxN, vector<int>(logN, -1));
vector<int> p(maxN), d(maxN);

void dfs(int node, int par, int depth) {
	p[node] = par;
	lca[node][0] = par;
	d[node] = depth;

	for(auto u: adj[node]) {
		if(u != par) {
			dfs(u, node, depth + 1);
		}
	} 
}

void init_lca() {
	dfs(0, -1, 0);
	for(int j=1;j<logN;j++) {
		for(int i=0;i<maxN;i++) {
			if(lca[i][j-1] != -1) {
				int par = lca[i][j-1];
				lca[i][j] = lca[par][j-1];
			}
		}
	}
}

int get_lca(int u, int v) {
	if(d[u] > d[v]) {
		swap(u,v);
	}

	int diff = d[v] - d[u];

	while(diff > 0) {
		int i = log2(diff);
		v = lca[v][i];
		diff -= (1LL<<i);
	}

	if(u == v) {
		return u;
	}

	for(int i=logN-1;i>=0;i--) {
		if(lca[u][i] != -1 and lca[u][i] != lca[v][i]) {
			u = lca[u][i];
			v = lca[v][i];
		}
	}

	return p[u];
}

int get_nth_parent(int u, int n) {
	while(n > 0) {
		int i = log2(n);
		u = lca[u][i];
		n -= (1LL << i);
	}

	return u;
}


int calculate_node(int u, int v, int cost) {
	int ans = 1;

	int LCA = get_lca(u,v);

	int d1 = d[u] - d[LCA];
	int d2 = d[v] - d[LCA];

	if(d1 + d2 <= cost) {
		ans = v + 1;
	}
	else {
		if(cost > d1) {
			ans = get_nth_parent(v, d1 + d2 - cost) + 1;
		}
		else {
			ans = get_nth_parent(u, cost) + 1;
		}
	}

	return ans;
}

int main() {
	ios_base::sync_with_stdio(0); cin.tie(0); cout.tie(0);

	int n;
	cin >> n;
	for(int i=0;i<n-1;i++) {
		int u,v;
		cin >> u >> v;
		--u;--v;
		adj[u].push_back(v);
		adj[v].push_back(u);
	}


	init_lca();

	int q;
	cin >> q;
	while(q--) {
		int a,b,c;
		cin >> a >> b >> c;
		--a;--b;
		cout << calculate_node(a,b,c) << '\n';
	}
}