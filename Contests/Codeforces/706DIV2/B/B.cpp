...
void run_cases() {
    ...
    int inserted = -1;
    for(int i=1;i<=k;i++) {
        int maximum = *prev(ele.end());
        int mex_val = *mex.begin();
        // debug() << imie(maximum) imie(mex_val);
        inserted = (maximum + mex_val + 1) / 2;
        // debug() << imie(inserted);
        if(inserted == prev_inserted) {
            break;
        } 
        ele.insert(inserted);
        if(mex.count(inserted)) {
            mex.erase(mex.find(inserted));
        }
