...
int main() {
    ...
    // for(int i=0;i<n;i++) {
    //     cout << p[i] << " " << lcp[i] << " " << s.substr(p[i]) << '\n';
    // }
    int sublen = 0;
    int pos = 0;
    // define class 1 and class 2, how to define ?
    // if suffix index lies 0 and s.length() - 1 then class 1 
    // else if suffix index lies between s.length() + 1 and n-1 then class 2
    vector<int> cls(n);
    for(int i=2;i<n;i++) {
        if(p[i] >= 0 and p[i] <= slen - 1) {
            cls[i] = 1;
        }
        else{
            cls[i] = 2;
