...
void run_cases() {
    ...
    min_cost_flow<int64_t, int64_t> graph;
    graph.init(n + 4);
    ...
    for(auto u: occupied) {
        graph.add_directional_edge(source, u, 1, 0);
    }
    for(auto u: not_occupied) {
        graph.add_directional_edge(u, sink, 1, 0);
    }
    for(auto u: occupied) {
        for(auto v: not_occupied) {
            graph.add_directional_edge(u, v, 1, abs(u - v));
        }
    }

    cout << graph.solve_min_cost_flow(source, sink).second << '\n';
}
