...
void run_cases() {
    ...
    for(int i = 0; i < N; i++) {
        ...
        if(left[i] != -1) {
            tree.add_bidirectional_edge(left[i], i);
            parent[left[i]] = i;
        }
        if(right[i] != -1) {
            tree.add_bidirectional_edge(right[i], i);
            parent[right[i]] = i;
        }
    }

    vector<int64_t> unmarked(N, 1);
    tree.build({0}, unmarked);
    ...
    vector<char> next_unique(N, '?');
    if(N - 2 >= 0) {
        next_unique[order[N - 2]] = value[order[N - 1]];
    }

    for(int i = N - 3; i >= 0; i--) {
        if(value[order[i]] != value[order[i + 1]]) {
            next_unique[order[i]] = value[order[i + 1]];
        } else {
            next_unique[order[i]] = next_unique[order[i + 1]];
        }
    }

    debug() << imie(next_unique);

    for(int i = 0; i < N - 1; i++) {
        int u = order[i];
        int v = order[i + 1];
        debug() << imie(i) imie(value[u]) imie(next_unique);
        if(value[u] < next_unique[u]) {
            int64_t toMark = tree.query(u, 0);
            debug() << imie(u) imie(v) imie(toMark);
            if(toMark <= K) {
                debug() << imie(u);
                tree.update(u, 0, 0);
                K -= toMark;
            }
        } else if(value[u] > next_unique[u] && tree.query(u, u) == 1) {
            tree.update(u, u, INF);
        }
