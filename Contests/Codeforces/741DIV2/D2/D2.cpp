...
void run_cases() {
    ...
    auto get_sum = [&](int l, int r) {
        ...
        if(l % 2 == 0) {
            sum = even.query(l, r) - odd.query(l, r);
        } else {
            sum = odd.query(l, r) - even.query(l, r);
        }

        return sum;
    };

    auto get_partition = [&](int l, int r) {

        int start = l, end = r;

        while(r > l + 1) {
            int m = (l + r) / 2;
            int left_sum = get_sum(start, m);
            int right_sum = get_sum(m + 1, end);

            debug() << imie(left_sum) imie(right_sum) imie(m);
    ...
    while(Q--) {
        ...
        int sum = abs(get_sum(l, r));

        if(sum == 0) {
            cout << 0 << '\n';
        } else if(sum % 2 == 0) {
            cout << 2 << '\n';
            cout << r << " " << get_partition(l, r - 1) + 1 << '\n';
        } else {
            cout << 1 << '\n';
            cout << get_partition(l, r) + 1 << '\n';
        }
    }
}
