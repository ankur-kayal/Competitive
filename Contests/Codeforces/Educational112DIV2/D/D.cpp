...
void run_cases() {
    ...
    // {a, b, c} 6 permutations 6 patterns 6 fenwick trees
    ...
    vector<fenwick_tree<int>> tree(6);

    for(int i = 0; i < 6; i++) {
        tree[i].init(N + 100);
        for(int j = 0; j < N; j++) {
            int val = (S[j] == patterns[i][j] ? 0 : 1);
            tree[i].set(j, val);
        }
    }
