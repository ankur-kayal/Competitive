...
void run_cases() {
    ...
    mod_int ans = factorial(N);

    sort(A.begin(), A.end());

    vector<int64_t> uniq = A;
    uniq.resize(unique(uniq.begin(), uniq.end()) - uniq.begin());
    int64_t largest = uniq.back();
    uniq.pop_back();
    if(uniq.empty()) {
        cout << ans << '\n';
        return;
    ...
    int second_largest = uniq.back();
    int64_t count_second_largest = count(A.begin(), A.end(), second_largest);
    int64_t count_largest = count(A.begin(), A.end(), largest);
    ...
    debug() << imie(count_largest) imie(count_second_largest);

    

    int64_t other_part = N - 1 - count_second_largest;

    mod_int extra = factorial(count_second_largest) * choose(N, other_part) * factorial(other_part);
