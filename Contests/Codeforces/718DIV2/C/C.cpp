...
void run_cases() {
    ...
    auto make_placement = y_combinator([&](auto make_placement, int row, int col, int value) -> void {
        if(rem == 0) {
            return;
        }
        if(col - 1 >= 0 and grid[row][col-1] == 0) {
            // left is empty and should be filled asap
            grid[row][col-1] = value;
            rem--;
            make_placement(row, col-1, value);
        }
        if(rem == 0) {
            return;
        }
        if(row - 1 >= 0 and grid[row-1][col] == 0) {
            // up is empty and should be filled asap
            grid[row-1][col] = value;
            rem--;
            make_placement(row - 1, col, value);
        }
        if(rem == 0) {
            return;
        }
        if(row + 1 < n and grid[row+1][col] == 0) {
            // down is empty and should be filled asap
            grid[row+1][col] = value;
            rem--;
            make_placement(row + 1, col, value);
        }
        ...
        if(col + 1 < n and grid[row][col+1] == 0) {
            // right is empty and should be filled asap
            grid[row][col+1] = value;
            rem--;
            make_placement(row, col+1, value);
        }
