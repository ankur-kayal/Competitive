...
void solve() {
    ...
    FOR(i, 2, N) {
        ...
        if (in[y] < in[x] && in[x] < out[y]) B -= sub[v];
        if (in[x] < in[i] && in[i] < out[x]) {
            A -= sub[i];
            x = i;
        } else if (in[i] < in[x] && in[x] < out[i]) {
            A = 0;
        } else if (in[y] < in[i] && in[i] < out[y]) {
            if (in[v] < in[i] && in[i] < out[v]) {
                x = -1; y = -1;
            } else {
                B -= sub[i];
                y = i;
            }
        } else if (in[i] < in[y] && in[y] < out[i]) {
            B = 0;
        } else {
            x = -1; y = -1;
        }
    ...
    FOR(i, 2, N) {
        ...
        if (in[cv] <= in[i] && in[i] < out[cv]) {

        } else if (in[i] <= in[cv] && in[cv] < out[i]) {
            cout << 0 << " ";
        } else {
    ...
    /*ll sum = 0;
    F0R(i, N) {
        ll cur = N-1; cur = cur*(cur-1)/2;
        trav(a, graph[i]) {
            cur -= sub[a]*(sub[a]-1)/2;
        }
        cur -= (N-sub[i])*(N-sub[i]-1)/2;
        cout << cur-sum << 
    }
    ll cur = N; cur = cur*(cur-1)/2;
    cout << cur-sum << nl;*/
