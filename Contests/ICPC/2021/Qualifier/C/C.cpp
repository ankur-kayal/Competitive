...
void run_cases(int test) {
	...
	while(Q--) {
		...
		int maxDepth = -1;
		int maxDepthVertex = -1;

		for(auto u: nodes) {
			if(tree.depth[u] > maxDepth) {
				maxDepth = tree.depth[u];
				maxDepthVertex = u;
			}
		}
		...
		for(auto u: nodes) {
			right = max(right, {tree.get_dist(u, maxDepthVertex), u});
		}

		int answer = 0;

		for(auto u: nodes) {
			int common_node = tree.get_common_node(maxDepthVertex, right.second, u);
			answer = max(answer, tree.get_dist(common_node, u));
		}
