...
void run_cases() {
    ...
    mod_int numerator = 0, denominator = 1;
    for(auto u: a) {
        denominator *= (u + 1);
    }
    ...
    for(int i=0;i<N;i++) {
        polynomials.push_back(vector<mod_int>{a[i] + 1, -2});
    }
    vector<mod_int> G = ntt.mod_multiply_all(polynomials);

    vector<mod_int> f1(N + 1), f2(N + 1), f3(N + 1);

    for(int i=0;i<=N;i++) {
        f1[i] = inv_factorial(i);
    }
    mod_int tmp = m;
    for(int i=0;i<=N;i++) {
        f2[i] = (tmp * inv_factorial(i + 1));
        tmp *= m;
    }

    for(int i=0;i<=N;i++) {
        f3[i] =  inv_factorial(i + 1);
    }

    polynomials.clear();
    polynomials = {f1, f2, ntt.mod_inverse(f3)};
    vector<mod_int> A = ntt.mod_multiply_all(polynomials);
