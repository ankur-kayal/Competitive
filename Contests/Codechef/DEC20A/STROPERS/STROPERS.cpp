    ...
    while(t--) {
        ...
        for(int i=0;i<n;i++) {
            ...
            for(int j=i;j<n;j++) {
                ...
                if(ones == 0) {
                    ans.insert(mp(mp(length, ones), mp(length, length)));
                }
                else if(ones == 1) {
                    shift = j;
                    prev = j;
                    // debug() << imie(i) imie(j);
                    ans.insert(mp(mp(length, ones), mp(0, curr - i)));
                    // debug() << imie(ans);
                }
                else {
                    // debug() << imie(shift) imie(prev) imie(ones);
                    if(length == 3) {
                        debug() << imie(curr) imie(prev) imie(i) imie(j);
                    }
                    int zero_gap = curr - prev;
                    ans.insert(mp(mp(length, ones), mp(zero_gap, 0)));
                }
            }
        }
        debug() << imie(s);
