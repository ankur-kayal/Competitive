...
void run_cases() {
    ...
    for(int i=0;i<n;i++) {
        vector<Mint> tmp = {a[i] + 1, -2};
        polys.push_back(tmp);
    }

    G = mod_multiply_all(polys);
    ...
    vector<Mint> A = f1 * f2 * inverse(f3);
