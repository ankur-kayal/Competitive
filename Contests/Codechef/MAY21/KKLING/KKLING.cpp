...
void run_cases() {
    ...
    vector<int> closest_leaf(N, inf);
    vector<int> dp_transition(N, inf);
    ...
    function <void(int,int)> find_parent_and_depths = [&](int node, int par) -> void {
        ...
        for(int child: adj[node]) {
            if(child != par) {
                ...
                closest_leaf[node] = min(closest_leaf[node], closest_leaf[child] + 1);
            }
        }
    };
    ...
    function<void(int,int)> dfs = [&](int node, int par) -> void {
        if(par != 0 && par != -1) {
            ...
            for(auto child: adj[par]) {
                if(child != node) {
                    if(child != parent[par])
                        dp_transition[node] = min(dp_transition[node], closest_leaf[child] + 1);
                    else 
                        for(auto u: adj[parent[par]]) {
                            if(u != par) {
                                debug() << imie(node) imie(u) imie(par);
                                dp_transition[node] = min(dp_transition[node], closest_leaf[u] + 1);
                            }
                        }
                    // debug() << imie(closest_leaf[child]) imie(child) imie(par) imie(dp_transition[node]);
                }
            }
        }
        for(int child: adj[node]) {
    ...
    for(int day=1;day<=N;day++) {
        ...
        for(int assassin : assassins) {
            ...
            int next_town = start_killing(assassin, day);
            if(next_town == -1) {
