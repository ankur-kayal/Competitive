...
void run_cases() {
    ...
    while(Q--) {
        ...
        // find the first end of the longest path among the nodes
        int first = -1;
        int max_dist = -1;
        for(int i = 0; i < K; i++) {
            int current_dist = tree.get_dist(query_nodes[0], query_nodes[i]);
        ...
        // find the second end of the longest path among the nodes
        int second = -1;
        max_dist = -1;
        for(int i = 0; i < K; i++) {
            int current_dist = tree.get_dist(query_nodes[i], first);
            if(current_dist > max_dist) {
                second = query_nodes[i];
                max_dist = current_dist;
            }
        }

        // check if all nodes lie in the path from first to second
        bool all_in_path = true;

        for(auto u: query_nodes) {
            if(!tree.on_path(u, first, second)) {
                all_in_path = false;
                break;
            }
        }

        cout << (all_in_path ? "YES" : "NO") << '\n';
    }
}
