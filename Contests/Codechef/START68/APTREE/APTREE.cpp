...
struct segment {
    ...
    int64_t prefix_difference;
    int64_t prefix_count;
    int64_t suffix_difference;
    int64_t suffix_count;
    int64_t answer;

    // Make sure that the default constructor is the identity segment
    segment(int64_t _answer = 0, int64_t _first_element = 0, int64_t _last_element = 0, int64_t _prefix_difference = 0, int64_t _suffix_difference = 0,
    	int64_t _prefix_count = 0, int64_t _suffix_count = 0) {
    	answer = _answer;
    	first_element = _first_element;
    	last_element = _last_element;
    	prefix_difference = _prefix_difference;
    	suffix_difference = _suffix_difference;
    	prefix_count = _prefix_count;
    	suffix_count = _suffix_count;
    }	
    ...
    void apply(int length, const segment_change &change) {
        if(change.has_change()) {
            answer = change.to_set;
        }
    }

    void join(const segment &other) {
    	int64_t merging_answer = 0;
    	int64_t merging_suffix_count = 0, merging_prefix_count = 0;
    	if (suffix_difference == other.prefix_difference) {
    		merging_answer = suffix_count + other.prefix_count;
    	}
    	if (other.first_element - last_element == other.prefix_difference) {
    		merging_answer = max(merging_answer, other.prefix_count + 1);
    		merging_prefix_count = 
    	}

    	if (other.first_element - last_element == suffix_difference) {
    		merging_answer = max(merging_answer, suffix_difference + 1);
    	}
    	int64_t merging_suffix_count = 1
        if (answer == 0)
        suffix_difference = other.suffix_difference;
        suffix_count = max({other.suffix_count, merging_suffix_count});
        prefix_count = max({prefix_count, merging_prefix_count});
        last_element = other.last_element;
        answer = max({answer, other.answer, merging_answer});
    }
