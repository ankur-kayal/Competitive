...
void run_cases() {
    ...
    vector<mod_int> poly(maxA + 1);
    for(auto u: A) {
        poly[u]++;
    }

    poly = ntt.mod_multiply(poly, poly);

    int64_t pairwise_sum = 0;
    for(int i = 1; i < poly.size(); i++) {
        pairwise_sum += int64_t(poly[i]) * i;
    }

    int64_t total_sum = accumulate(A.begin(), A.end(), 0LL);
