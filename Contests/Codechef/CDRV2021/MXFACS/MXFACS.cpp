...
int compute_factors(vector<pair<int64_t, int>> prime_factors) {
    int ans = 1;
    for(auto [_, e] : prime_factors) {
        ans *= (1 + e);
    }
    return ans;
}
...
void run_cases() {
    ...
    for(int i = 0; i < prime_factors.size(); i++) {
        prime_factors[i].second--;
        answers.push_back({-compute_factors(prime_factors), prime_factors[i].first});
        prime_factors[i].second++;
    }

    sort(answers.begin(), answers.end());

    cout << answers[0].second << '\n';
}
