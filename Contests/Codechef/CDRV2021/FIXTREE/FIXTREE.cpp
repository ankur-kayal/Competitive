...
void run_cases() {
    ...
    for(int i = 0; i < Q; i++) {
        ...
        queries.push_back({ch, x, w});
    }

    heavy_light_decomposition tree;
    tree.init(total_vertices);

    vector<vector<int>> adj(total_vertices);
    ...
    for(int i = 0; i < Q; i++) {
        ...
        if(ch == 1) {
            ...
            int w = queries[i][2] - 1;
            id[x] = extra++;
            tree.add_bidirectional_edge(id[x], id[w]);
        }
    }

    for(int i = 1; i < N; i++) {
        tree.add_bidirectional_edge(parent[i], i);
    }

    vector<int64_t> initial_values(total_vertices);
    for(int i = 0; i < N; i++) {
        initial_values[i] = value[i];
    }

    tree.build({0}, initial_values);
    // return;
    ...
    for(int i = 0; i < Q; i++) {
        ...
        if(ch == 1) {
            int x = queries[i][1] - 1;
            int w = queries[i][2] - 1;
            int64_t val = tree.query(id[x], id[x]);
            id[x] = extra++;
            tree.update(id[x], id[x], val);
        } else if(ch == 2) {
            int x = queries[i][1] - 1;
            int k = queries[i][2];
            tree.update(id[x], id[x], k);
        } else {
            int x = queries[i][1] - 1;
            int y = queries[i][2] - 1;
            cout << tree.query(id[x], id[y]) << '\n';
        }
    }
}
