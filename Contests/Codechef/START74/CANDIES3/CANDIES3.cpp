...
void run_cases() {
	...
	vector<int64_t> candies(M + 1);

	for (auto u: A) {
		vector<int64_t> factors = generate_factors(prime_factorize(u), true);
		for (int i = 0; i < int(factors.size()); i++) {
			candies[factors[i] - 1] += (u / factors[i]);
			if (i > 0) {
				candies[factors[i - 1] - 1] -= (u / factors[i]);
			}
		}
		debug() << imie(u) imie(candies);
	}

	debug() << imie(candies);

	int64_t answer = C[M - 1] * candies[M - 1];
	for (int i = M - 2; i >= 0; i--) {
		candies[i] += candies[i + 1];
		answer = max(answer, C[i] * candies[i]);
	}
