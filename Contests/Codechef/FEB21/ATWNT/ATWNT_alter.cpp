    ...
    if(children[node] != 0) {
        if(tasks % children[node] == 0) {
            for(auto child: adj[node]) {
                if(child != p[node])
                solve(renumber[child], tasks / children[node]);
            }
        }
        else {
            debug() << imie(node) imie(tasks);
            ans += tasks;
        }
    }
}
...
void run_cases() {
    ...
    while(Q--) {
        ...
        int ignored = 0;
        tasks.push({renumber[v], W});
        while(!tasks.empty()) {
            auto task = tasks.front();
            tasks.pop();
            int vertex = task.first;
            int current_task = task.second;

            if(children[vertex] != 0) {
                if(current_task % children[vertex] == 0) {
                    for(int child: adj[vertex]) {
                        if(child != p[vertex]) {
                            tasks.push({renumber[child], current_task / children[vertex]});
                        }
                    }
                }
                else {
                    ignored += current_task;
                }
            }
        }
        // solve(renumber[v], W);
