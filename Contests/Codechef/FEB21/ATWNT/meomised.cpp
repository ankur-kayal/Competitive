...
vector<gp_hash_table<int, int>> leafs;
int ans = 0;
...
void dfs(int node, int depth) {
    ...
    for(int child : adj[node]) {
        if(child != p[node]) {
            ...
            if(children[node] != 1)
                for(auto u: leafs[renumber[child]]) {
                    if(u.first * children[node] <= 1000000)
                        leafs[node][u.first * children[node]] += u.second;
                }
        }
    }
    if(children[node] == 1) {
...
void solve(int node, int tasks) {
    ...
    if(tasks % factor[node] == 0) {
        ans = 0;
        return;
    }
    if(tasks % children[node] == 0)
        for(auto u: leafs[node]) {
            if(tasks % u.first == 0) {
                ans -= tasks / u.first * u.second;
            }
        }
}
...
void run_cases() {
    ...
    leafs = vector<gp_hash_table<int, int>>(n + 1);
    p = vector<int>(n + 1);
