...
mod_int large_choose(int64_t n, int64_t r) {
    ...
    if(n > MOD && n - r >= MOD) {
        return segment(n, r) * inv_factorial(r);
    } else if(n > MOD && n - r < MOD) {
        return 0;
    }

    return large_factorials[n] / (factorial(r) * large_factorials[n - r]);
}
...
void run_cases() {
    ...
    debug() << imie(vals);
    generate_factorials(vals);
    debug() << imie(large_factorials);
    large_factorials[0] = 1;
    vector<vector<mod_int>> polynomials;
    for(int i = 0; i < k; i++) {
        vector<mod_int> val(2);
        val[0] = mod_int(1) - mod_int(P[i]);
        val[1] = mod_int(P[i]) * large_choose(A[i], r);
