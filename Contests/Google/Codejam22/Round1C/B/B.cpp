...
vector<int64_t> roars;
void preprocess() {
    vector<vector<int64_t>> len(20);
    int64_t cnt = 1;
    for(int64_t i=1;i<=10000000;i++) {
        ...
        for(int64_t j=i + 1;;j++) {
            ...
            cnt++;
            roars.push_back(stoll(tmp));
            len[tmp.length()].push_back(stoll(tmp));
        }
    }
    for(int i=1;i<=18;i++) {
        debug() << imie(i) imie(len[i].size());
    }
}
...
void run_cases() {
    ...
    cin >> y;
    vector<int64_t> roars
    cout << *upper_bound(all(roars), y) << nl;
}
