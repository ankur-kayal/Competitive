...
struct gcd_t {
  ...
  gcd_t op(gcd_t& other) {
    return gcd_t(vgcd(val, other.val));
  }
...
vector<pair<ll, pair<ll, ll>>> edges[300005];
ll res[300005];
vector<pair<ll, ll>> at[300005];
segtree<gcd_t> st;

void dfs(ll v, ll p) {
	for (pair<ll, ll> x: at[v]) {
		res[x.s] = st.query(0, x.f).val;
	}
	
	for (pair<ll, pair<ll, ll>> x: edges[v]) {
		if (x.f != p) {
			st.update(x.s.f, x.s.f, x.s.s);
...
void solve(int tc = 0) {
	...
	for (ll i = 0; i < n - 1; i++) {
		...
		edges[x].push_back(pair{y, pair{l, r}});
		edges[y].push_back(pair{x, pair{l, r}});
	}
