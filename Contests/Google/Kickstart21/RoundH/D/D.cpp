...
void run_cases() {
    ...
    while(Q--) {
        ...
        if(tree.depth[v] < tree.depth[u]) {
            swap(u, v);
        }

        {
            vector<mod_int> occurs(N + 1);
            vector<mod_int> does_not_occurs(N + 1);

            occurs[1] = mod_int(K) / denominator;
            does_not_occurs[1] = mod_int(base - K) / denominator;
            ...
            for(int i = 0; i < q.size(); i++) {
                int x = q[i];
                for(int children : adj[x]) {

                    mod_int a = ((mod_int(A[children]) / denominator) * occurs[x]);
                    mod_int b = ((mod_int(B[children]) / denominator) * does_not_occurs[x]);
                    occurs[children] = a + b;
                        
                        does_not_occurs[children] = ((mod_int(base - A[children]) / denominator) * occurs[x]) + 
                                                    ((mod_int(base - B[children]) / denominator) * does_not_occurs[x]);
