...
int main() {
    ...
    sort(projects.begin(), projects.end(), [&](Project &project1, Project &project2) {
        return project1.completionDays < project2.completionDays;
    });
    ...
    for(int i = 0; i < projects.size(); i++) {
        ...
        if(fulfilled) {
            ...
            for(auto contributor: contributedProject.contributors) {
                for(auto [skillName, skill]: contributorSkillMapping[contributor]) {
                    skillBucket[skillName].insert({skill + 1, contributor});
                }
            }
        } else {
            for(auto contributor: contributedProject.contributors) {
                for(auto [skillName, skill]: contributorSkillMapping[contributor]) {
                    skillBucket[skillName].insert({skill, contributor});
                }
            }
        }
